...
// Konami Code Implementation for Admin Access
// Hide the admin toggle button by default
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.key === konamiCode[konamiIndex]) {
    ...
    // If the full sequence is entered, show admin button
    if (konamiIndex === konamiCode.length) {
      // Show the admin button with a cool effect
      toggleAdminButton.style.display = 'block';
      toggleAdminButton.classList.add('konami-activated');
      
      // Add a temporary animation class
      toggleAdminButton.classList.add('pulse-animation');
      setTimeout(() => {
        toggleAdminButton.classList.remove('pulse-animation');
      }, 2000);
      
      // Show a subtle notification
      updateStatus('Mode administrateur déverrouillé', 'success');
...
// Add CSS for the animation (add this to your CSS file or in a style tag)
